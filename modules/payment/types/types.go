...
var (
	...
	GovernanceAddress       = sdk.AccAddress(address.Module(ModuleName, []byte("governance"))[:EthAddressLength])
	ValidatorTaxPoolAddress = sdk.AccAddress(address.Module(ModuleName, []byte("validator-tax-pool"))[:EthAddressLength])
)
